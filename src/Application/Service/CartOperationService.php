...
use Crehler\InpostPay\Domain\Exception\{BasketSessionNotFoundException, InvalidBasketException, OrphanedBasketSessionException};
...
readonly class CartOperationService
{
    ...
    public function loadCartByBasketId(string $basketId, array $contextStates = []): Cart
    {
        ...
        try {
            ...
            // The InPost session outlives the Shopware cart (e.g. abandoned basket after a token change).
            // Signal 404 so InPost stops retrying and unbinds, instead of hammering a retryable 422.
            throw OrphanedBasketSessionException::cartNotFound($basketId, $cartToken, $e);
        } catch (Throwable $e) {
