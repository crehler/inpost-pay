...
use Crehler\InpostPay\Domain\Exception\{BasketSessionNotFoundException, InvalidBasketException, InvalidPhoneNumberException, OrphanedBasketSessionException, UnsupportedEventTypeException};
...
readonly class BasketService
{
    ...
    public function loadBasketData(string $basketId): InpostBasket
    {
        ...
        } catch (OrphanedBasketSessionException $e) {
            $this->dropOrphanedSession($basketId);
            throw $e;
    ...
    /**
     * Removes an InPost basket session whose Shopware cart no longer exists, so we stop
     * tracking a dead basket. Ignores a missing session (e.g. removed by a concurrent request).
     */
    private function dropOrphanedSession(string $basketId): void
    {
        try {
            $this->sessionService->deleteSessionByBasketId($basketId);
            $this->logger->info('Removed orphaned InPost basket session (cart no longer exists)', [
                'basketId' => $basketId,
            ]);
        } catch (Throwable $e) {
            // Do not rethrow: cleanup failure must not turn the intended 404 into a 500 for InPost.
            // Still surface it (concurrent removal, DB error, etc.) instead of swallowing silently.
            $this->exceptionLogger->warning('Failed to drop orphaned InPost basket session', $e, [
                'basketId' => $basketId,
            ]);
        }
    }
