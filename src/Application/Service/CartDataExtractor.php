...
readonly class CartDataExtractor
{
    ...
    public function extractProducts(Cart $cart, SalesChannelContext $context): array
    {
        ...
        foreach ($productLineItems as $lineItem) {
            ...
            $isDigital = $this->isDigitalProduct($lineItem);
    ...
    public function extractProductsToOrderLines(Cart $cart, SalesChannelContext $context): array
    {
        ...
        foreach ($productLineItems as $lineItem) {
            ...
            $orderLine = new OrderLine(
                ...
                type: $this->isDigitalProduct($lineItem)
                    ? ProductType::DIGITAL
                    : ProductType::PRODUCT,
                quantity: $quantityVO,
                basePrice: $basePrice,
    ...
    private function loadProductMedia(array $productIds, SalesChannelContext $context): array
    {
        ...
        foreach ($products as $product) {
            ...
            $productImages = [];
            foreach ($mediaItems as $productMedia) {
                if ($coverId !== null && $productMedia->getId() === $coverId) {
                    continue;
                }

                $productImage = $this->buildProductImage($productMedia);
            ...
            $images[$product->getId()] = $productImages;
        }

        return $images;
    }

    private function buildProductImage(ProductMediaEntity $productMedia): ?ProductImage
    {
        ...
        return new ProductImage(smallSize: $small, normalSize: $normal);
    }
    ...
    private function isDigitalProduct(LineItem $lineItem): bool
    {
        return $lineItem->hasState(State::IS_DOWNLOAD);
    }
}
