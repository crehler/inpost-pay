...
use Shopware\Core\Checkout\Shipping\SalesChannel\{AbstractShippingMethodRoute, SortedShippingMethodRoute};
use Shopware\Core\Content\Media\MediaEntity;
use Shopware\Core\Content\Product\{ProductCollection, State};
...
readonly class CartDataExtractor
{
    ...
    public function __construct(
        ...
        #[Autowire(service: SortedShippingMethodRoute::class)]
    ...
    public function extractDeliveryOptions(Cart $cart, SalesChannelContext $context, array $products): array
    {
        ...
        // Price under a non-COD context so the COD surcharge is not double-counted.
        ...
        foreach (DeliveryType::cases() as $deliveryType) {
            ...
            // First configured method that is actually available for this cart.
            $deliveryPrice = null;
    ...
    public function extractProducts(Cart $cart, SalesChannelContext $context): array
    {
        ...
        $productMedia = $this->loadProductMedia($productIds, $context);
        ...
        foreach ($productLineItems as $lineItem) {
            ...
            $product = new BasketProduct(
                ...
                basePrice: $basePrice,
                quantity: $quantity,
                productType: $lineItem->hasState(State::IS_DOWNLOAD) ? ProductType::DIGITAL : ProductType::PRODUCT,
                ...
                additionalProductImages: $productMedia[$lineItem->getReferencedId()] ?? [],
    ...
    public function extractProductsToOrderLines(Cart $cart, SalesChannelContext $context): array
    {
        ...
        foreach ($productLineItems as $lineItem) {
            ...
            $orderLine = new OrderLine(
                ...
                type: $lineItem->hasState(State::IS_DOWNLOAD) ? ProductType::DIGITAL : ProductType::PRODUCT,
                quantity: $quantityVO,
                basePrice: $basePrice,
                ean: null,
    ...
    private function loadProductMedia(array $productIds, SalesChannelContext $context): array
    {
        ...
        foreach ($products as $product) {
            ...
            $mediaByProduct[$product->getId()] = $images;
        }
