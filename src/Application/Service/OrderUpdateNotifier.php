...
readonly class OrderUpdateNotifier
{
    public function __construct(
        ...
        private OrderStatusDescriptionMapper $statusMapper,
    ...
    private function buildStatusDescription(OrderEntity $order): string
    {
        ...
        if ($deliveries !== null && $deliveries->count() > 0) {
            ...
            if ($shipped > 0) {
                ...
                return $this->statusMapper->mapDeliveryStateToPolish($deliveryState, $shipped, $total);
            }
        }

        $transactionState = $order->getTransactions()?->first()?->getStateMachineState()?->getTechnicalName();
        if ($transactionState !== null) {
            return $this->statusMapper->mapToPolish($transactionState);
        }

        return $this->statusMapper->mapToPolish(
            $order->getStateMachineState()?->getTechnicalName() ?? ''
        );
    }
}
